...
type DelegationMatcherFunc func(
	ctx context.Context,
	aud did.DID,
	cmd ucan.Command,
	sub did.DID,
) iter.Seq2[ucan.Delegation, error]
...
type DelegationListerFunc func(
	ctx context.Context,
	aud did.DID,
	cmd ucan.Command,
	sub did.DID,
...
func ProofChain(
	ctx context.Context,
	matchDelegations DelegationMatcherFunc,
	aud did.DID,
	cmd ucan.Command,
	sub did.DID,
) ([]ucan.Delegation, []cid.Cid, error) {
...
func proofChain(
	...
	for d, err := range matchDelegations(ctx, aud, cmd, sub) {
		...
		if d.Subject().Defined() && d.Subject() == d.Issuer() {
